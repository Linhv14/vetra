...
const ChartOptionButton: React.FC<ChartOptionButtonProps> = ({ isOpen, chartID, onClose }) => {
  const optionButtonRef = React.useRef<HTMLDivElement | null>(null);

  const handleOuterClick = (e: MouseEvent) => {
    const targetNode = e.target as Node;
    if (optionButtonRef.current && !optionButtonRef.current.contains(targetNode)) {
      onClose();
    }
  };

  const downloadChartAsImage = (chartID: string) => {
    ApexCharts.exec(chartID, "dataURI").then(({ imgURI, blob }: { imgURI: string, blob: Blob }) => {
